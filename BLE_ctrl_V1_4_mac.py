...
3) 运行程序：
   python3 BLE_ctrl_V1_4_mac.py --name ROVER_BLE

4) 如果你的 BLE 设备名称不是 ROVER_BLE，改成你的设备名：
   python3 BLE_ctrl_V1_4_mac.py --name 你的设备名

5) 如果 Mac 提示蓝牙权限：
   System Settings → Privacy & Security → Bluetooth
   允许 Terminal / VS Code / 你正在使用的终端访问蓝牙。
...
# ============================================================
#  macOS 非阻塞键盘读取（不按键时立即返回空字符串）
#  使用 termios + tty + select；这是 macOS / Linux 的终端键盘读取方式
# ============================================================
_old_terminal_settings = None

def setup_keyboard():
    """把终端切到 cbreak 模式，使按键不用回车也能被读取。"""
    global _old_terminal_settings
    if sys.stdin.isatty():
        _old_terminal_settings = termios.tcgetattr(sys.stdin)
        tty.setcbreak(sys.stdin.fileno())

def restore_keyboard():
    """程序退出时恢复终端设置，避免终端显示异常。"""
    global _old_terminal_settings
    if _old_terminal_settings is not None and sys.stdin.isatty():
