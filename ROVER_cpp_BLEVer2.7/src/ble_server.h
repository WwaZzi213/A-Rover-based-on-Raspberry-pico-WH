...
// -------------------------------------------------------
//  运行时配置（由 CONFIG 特征写入更新）
//  CONFIG 包格式：[0]=命令类型, [1]=参数值
// -------------------------------------------------------
#define CFG_CMD_SET_THRESHOLD  0x01   // [1] = 避障阈值 cm（≥5）
#define CFG_CMD_SET_MODE       0x02   // [1] = 0:手动  1:自动

typedef enum { DRIVE_MODE_MANUAL = 0, DRIVE_MODE_AUTO = 1 } DriveMode;

uint8_t   ble_server_get_obstacle_cm(void);   // 当前避障阈值
DriveMode ble_server_get_drive_mode(void);    // 当前驾驶模式
