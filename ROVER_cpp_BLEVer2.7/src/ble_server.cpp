...
#define HANDLE_CFG     ATT_CHARACTERISTIC_12345678_1234_1234_1234_1234567890ae_01_VALUE_HANDLE
...
// 运行时配置
static volatile uint8_t   s_obstacle_cm  = OBSTACLE_STOP_DISTANCE_CM;
static volatile DriveMode s_drive_mode   = DRIVE_MODE_MANUAL;
                        ...
                        uint16_t buffer_size)
{
    ...
    } else if (attribute_handle == HANDLE_CFG) {
        if (buffer_size >= 2) {
            uint8_t cfg_cmd = buffer[0];
            uint8_t cfg_val = buffer[1];
            if (cfg_cmd == CFG_CMD_SET_THRESHOLD && cfg_val >= 5) {
                s_obstacle_cm = cfg_val;
                printf("[CFG] obstacle threshold = %u cm\n", cfg_val);
            } else if (cfg_cmd == CFG_CMD_SET_MODE) {
                s_drive_mode = (cfg_val == 1) ? DRIVE_MODE_AUTO : DRIVE_MODE_MANUAL;
                printf("[CFG] drive mode = %s\n", cfg_val ? "AUTO" : "MANUAL");
            }
        }
                               ...
                               uint16_t size)
{
    ...
    case HCI_EVENT_DISCONNECTION_COMPLETE:
        ...
        s_connected      = false;
        s_notify_enabled = false;
        s_drive_mode     = DRIVE_MODE_MANUAL;   // 断连后回到手动模式（安全）
        start_advertising();
        break;
...
uint8_t   ble_server_get_obstacle_cm(void) { return s_obstacle_cm; }
DriveMode ble_server_get_drive_mode(void)  { return s_drive_mode;  }